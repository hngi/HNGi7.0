...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!filter_var($_POST['cv_link'], FILTER_VALIDATE_URL)) {
    $error['cv_link'] = "Invalid CV link";
  }
  if (empty($_POST['github_link']) && empty($_POST['dribble_link'])) {
    $error['portfolio'] = "You have to input either github or dribble link";
  }
  if (!empty($_POST['dribble_link']) && !filter_var($_POST['dribble_link'], FILTER_VALIDATE_URL)) {
    $error['dribble_link'] = "Invalid dribble link";
  }

  if (!filter_var($_POST['fb_url'], FILTER_VALIDATE_URL)) {
    $error['fb_url'] = "Invalid facebook link";
  }
  if (!filter_var($_POST['twitter_url'], FILTER_VALIDATE_URL)) {
    $error['twitter_url'] = "Invalid twiiter link";
  }
  if (!empty($_POST['github_link']) && !filter_var($_POST['github_link'], FILTER_VALIDATE_URL)) {
    $error['github_link'] = "Invalid github link";
  }
  ...
  Mentors::emailExists($_POST['email']) ? $error['email'] = "Email has been used to apply before" : false;
  if (empty($error)) {
    $area_of_expertise = "";
      ...
      if ($fileError === 0) {
        ...
        if ($fileSize < 400000) {
          ...
          if (move_uploaded_file($fileTmpName, $fileDestination)) {
            ...
            $mentor->link_to_github = $database->escape_string($_POST['github_link']);
            ...
            $mentor->dribble_link = $database->escape_string($_POST['dribble_link']);
